...
1. **Bot をサーバーに招待**  
   - 事前に Developer Portal で Public Bot をオン、OAuth2 スコープ・権限を設定し、生成した招待リンクから Bot をサーバーに追加。  
   - （「アプリを追加」ボタンから手軽に招待することも可能です。）
...
3. **ユーザーは \`settings\` チャンネルで言語設定**  
   - \`settings\` チャンネル内の指示に従い、自分の **デフォルト言語** を設定する。  
...
6. **必要があれば \`bot-announcements\` で重要通知**  
   - 管理者が \`bot-announcements\` を使い、サーバー全体（Bot が参加しているすべてのサーバー）に向けて一斉通知を行うことが可能。  
   - ただし、このチャンネルは主に Bot オーナーが運営告知をするときに使う場合が多い。
`,
