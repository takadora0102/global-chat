/**
 * index.js – Global Chat Bot (2025-05-28 Announcement Follow Feature)
 * -----------------------------------------------------------------------------
 * • グローバルチャット中継（Hub 経由）
 * • インライン自動翻訳（Google 非公式 API）
 * • 時差タグ（UTC±12：都市名＋国旗付き）＋🌏 自動判定ボタン
 * • 国旗リアクション翻訳（多言語対応）
 * • /setup         … カテゴリ／チャンネル自動作成＋設定 UI ＋ アナウンスチャンネルのフォロー登録
 * • /announce      … OWNER_ID のみが全サーバーに一斉通知（deferReply 対応）
 */
...
async function handleSetup(interaction) {
  ...
  if (process.env.SOURCE_ANNOUNCE_CHANNEL_ID) {
    try {
      ...
      else console.error('❌ Follow registration failed', await res.text());
    } catch (err) {
      console.error('❌ Follow registration error', err);
    }
  }
  ...
  const rowTz = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .setPlaceholder('サーバー標準タイムゾーンを選択')
